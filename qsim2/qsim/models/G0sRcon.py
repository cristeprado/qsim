...
class Model(ModelTemplate):
    ...
    def calc_b_h(self,dm,t):
        b_h=(-1/dm.mu)*(log(( self.old.S_vi*self.curr.phi_hvi*exp(dm.mu*self.curr.b_h_vi)/(dm[t].H_h*self.curr.phi_hvi*exp(dm.mu*(self.old.b_h+self.curr.b_h_vi))).sum(axis=0)[newaxis,:]).sum(axis=1)[:,newaxis]))

        return(b_h)

    def calc_phi_hvi(self,dm,t):
        phi_hvi_1=1/(1+((1-dm.nu)/dm.nu)*exp(dm.w*(-(self.old.b_h+self.curr.b_h_vi))))
        phi_hvi_2=1/(1+((1-dm.nu)/dm.nu)*exp(dm.w*((self.old.b_h+self.curr.b_h_vi-dm.Z_h))))
        print phi_hvi_2
        print 'bh',dm[t-1].b_h
