...
from qsim import SimulationStep, Simulation, MatrixReader, \
        ParameterSetReader, VerticalVectorReader, HorizontalVectorReader, ReportGenerator, Plotter
from qsim.models.test_model import Model as TestModel
...
class ReaderTests(BaseTest):
    ...
    def test_read_parameters(self):
        ...
        """
        reader = ParameterSetReader('data/test_data/parameters.txt')
    ...
    def test_read_vertical_vector(self):
        ...
        """
        reader = VerticalVectorReader('data/test_data/vertical.txt')
    ...
    def test_read_horizontal_vector(self):
        ...
        reader = HorizontalVectorReader('data/test_data/horizontal.txt')
        data_set = reader.get_data()
    ...
    def test_read_matrix(self):
        ...
        reader = MatrixReader('data/test_data/matrix.txt')
        data_set = reader.get_data()
...
class SimulationTest(BaseTest):
    ...
    def setUp(self):
        self.households_data = VerticalVectorReader(
               'data/test_data/households_data.txt').get_data()
        self.params = ParameterSetReader('data/test_data/parameters.txt').get_data()
        self.locations_data = HorizontalVectorReader(
               'data/test_data/locations_data.txt').get_data()
        self.probs_init = MatrixReader('data/test_data/data_P_h_vi_m1.txt').get_data()
...
class ReportGeneratorTest(BaseTest):
    ...
    def test_generate_reports(self):
        self.sim.run()
        repgen = ReportGenerator(self.sim,"output/test_output")
        repgen.generate_variable_reports()
        self.assertPathExists("output/test_output/report_H_h_vi.txt")
        repgen.generate_step_reports()
        self.assertPathExists("output/test_output/report_iters_t0.txt")
        repgen.store_results()
        self.assertPathExists("output/test_output/data.pkl")

class PlotterTest(BaseTest):
