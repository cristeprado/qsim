...
def plot2(dm):
    ...
    max_y=1.1*np.ceil(max([dm[ti].__dict__[plot_var][:,vi][dm[ti].__dict__[plot_var][:,vi]>0].sum() for vi in range(lenX) for ti in range(dm.T_MAX)]))
    min_y=1.1*np.floor(min([dm[ti].__dict__[plot_var][:,vi][dm[ti].__dict__[plot_var][:,vi]<0].sum() for vi in range(lenX) for ti in range(dm.T_MAX)]))
    ...
    def update(val):
        ...
        for h in range(dm.N_h):
            for vi in range(dm.N_vi):
                rect=pp[h][vi]
                tt=bar_labels[h][vi]
...
if __name__=="__main__":
    if len(sys.argv) > 1:
        fload=open(sys.argv[1]+".dat",'r')
    else:
        fload=open("dm.dat",'r')
    dm=pickle.load(fload)
    fload.close()
    plot2(dm)
